...
// --- Signed-in user chip ---
// When served by the Apps Script web app, the page injects window.SCRATCH_USER
// (the allowlisted email). On localhost there's no value, so the chip stays hidden.
function initUserChip() {
  const email = typeof window !== 'undefined' ? window.SCRATCH_USER : '';
  const chip = $('userChip');
  if (!chip) return;
  if (email) {
